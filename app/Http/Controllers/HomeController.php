...
//        $currentLocale = session('locale', config('app.locale'));
//
//        $cartItemsWithEvents = Product::with('event')->where('session_id', $sessionId)->get();
//
//        $cartItems = Product::where('session_id', $sessionId)->get();
//
...
//            return view('partner', ['showTestRecord' => true, 'cartItemsWithEvents' => $cartItemsWithEvents , 'eventsCount' =>  $eventsCount , 'totalItems' => $totalItems, 'cartCount' => $cartCount, 'totalOrders' => $totalOrders,  'likeCount' => $likeCount, 'use' => $use, 'ordersdata' => $ordersdata, 'userDataRecords' => $userDataRecords, 'currentLocale' => $currentLocale, 'events' => $events, $sessionIds=>'sessionIds', 'orders' => $orders]);
//        } elseif ($user->role_id == 2) {
//            return view('partner', ['showTestRecord' => true, 'cartItemsWithEvents' => $cartItemsWithEvents , 'eventsCount' =>  $eventsCount , 'totalItems' => $totalItems, 'cartCount' => $cartCount, 'totalOrders' => $totalOrders, 'likeCount' => $likeCount, 'ordersdata' => $ordersdata, 'use' => $use, 'currentLocale' => $currentLocale, 'userDataRecords' => $userDataRecords, 'events' => $events, $sessionIds=>'sessionIds', 'orders' => $orders]);
//        }
//    }
    ...
    {
        $theme = $request->query('theme', 'darwin');
        return redirect("/cabinet/profile?theme={$theme}");
    }
