...
@section('content')
    ...
    <div class="container" >
        <div class="row justify-content-center">
            <div class="container margin_60" style="margin-top:90px;">
                <div class="row">
                            ...
                            <div class="row" style="min-height:233px; border: 1px solid #dddddd; border-radius: 5px;">
                                <div class="col-lg-4 col-md-4">
                                    ...
                                    <div class="img_list">
                                        <a href="/{{$event->id}}">
                                            <img
                                                src="{{ $event->first_image_path ? $event->first_image_path : asset('default_image_path.jpg') }}"
                                                alt="Image">
                                        </a>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6">
                                    <div class="tour_list_desc">
                                        ...
                                        @if ($event->category != 4)
                                                ...
                                                <i style="font-size: 30px;" onclick="AddDoing({{ $event->id }});">
                                    ...
                                    <div class="price_list" style="font-size: small;">
                                        <div>
                                            ...
                                            <a href="/{{$event->id}}" type="button" data-toggle="modal" data-target="#bonusProgramModal" style="text-decoration: none; color: grey; background-color: #ffffff; padding: 10px 20px; border-radius: 5px; display: inline-block; border: 2px solid grey;" class="btn_1" target="_blank">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-gift-fill" viewBox="0 0 16 16">
...
<script>
    ...
    function likeButtonClicked(eventId) {
        fetch('/like', {
            ...
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer {{env('API_BEARER_TOKEN')}}`, // Отправка Bearer токена
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            },
    ...
    function AddDoing(id) {
        fetch('/doings/add', {
            ...
            .then(data => {
                if (data.success) {
                    alert('Событие добавлено в корзину!');
    ...
    function updateDoingCartModal() {
        fetch('/cart/doing/contents', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
            ...
            .then(data => {
                ...
                // Обновляем счетчик корзины
                let modalDoingCartCount  = document.getElementById('doing-cart-count');
                modalDoingCartCount.innerText = 'Кількість подій: '+data.cartDoingCount;

            })
            .catch(error => {
                console.error('Ошибка обновления корзины событий:', error);
            });
    }
    ...
    function redirectTo(goodId) {
        window.location.href = '/' + goodId;
    }
